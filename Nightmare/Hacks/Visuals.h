...
namespace Visuals {
    ...
    {
        if (*memory.disablePostProcessing != config.visuals.misc.disablePostProcessing)
            *memory.disablePostProcessing = config.visuals.misc.disablePostProcessing;
    }

    constexpr bool removeHands(const std::string_view modelName) noexcept
    {
        return config.visuals.misc.noHands && (modelName.find("arms") != std::string_view::npos);
    }

    constexpr bool removeSleeves(const std::string_view modelName) noexcept
    {
        return config.visuals.misc.noSleeves && (modelName.find("sleeve") != std::string_view::npos);
    }

    constexpr bool removeWeapons(const std::string_view modelName) noexcept
    {
        return config.visuals.misc.noWeapons && modelName.find("models/weapons/v_") != std::string_view::npos
    ...
    constexpr void skybox() noexcept
    {
        ...
        if (config.visuals.misc.skybox)
            memory.loadSky(skyboxes[config.visuals.misc.skybox]);
    }
};
